...
if (SERVER) then
	function ENT:Initialize()
		self:SetModel("models/neeewpackofprops/ingredient2.mdl")
		self:PhysicsInit(SOLID_VPHYSICS)
		self:SetMoveType(MOVETYPE_VPHYSICS)
		self:SetUseType(SIMPLE_USE)
		self.loopsound = CreateSound(self, "plats/elevator_move_loop1.wav")
		self.receivers = {}
		self:SetVar("table","cooking")
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		ix.util.drawText("Cooking Station", x, y, ColorAlpha(ix.config.get("color"), 255), 1, 1, nil, 255 * 0.65)
		ix.util.drawText("A station with all the appropriate tools and devices for cooking all manner of foods.", x, y + 16, ColorAlpha(color_white, 255), 1, 1, "ixSmallFont", 255 * 0.65)
	end
end